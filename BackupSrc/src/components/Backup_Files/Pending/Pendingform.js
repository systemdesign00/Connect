...
let newitemtotal = values.orderDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0) +  values.watageitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0) + 
 ...
 values.pieceitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0); 
  ...
  let purchase =  values.orderDetails.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0); 
...
// eslint-disable-next-line react-hooks/exhaustive-deps
    }, [JSON.stringify(values.orderDetails),JSON.stringify(values.oldorderDetails),JSON.stringify(values.watageitems),JSON.stringify(values.fancyitems),JSON.stringify(values.pieceitems)]);
...
*/
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.PENDINGORDER).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.PENDINGORDER).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
