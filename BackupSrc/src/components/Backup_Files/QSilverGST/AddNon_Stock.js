...
export default function AddNon_Stock(props, initialFValues) {
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && addorderedFoodItems.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, addorderedFoodItems])
    let today = new Date();
...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        ...
        setValues({
            ...values,
            addorderDetails: [...values.addorderDetails, x]
        })
...
const b = Number(rates) + Number(foodItem.tax);
...
/>
   { /* <TextField focused size="small" style = {{width: 100}}  label="foodItemName" name="foodItemName" id="foodItemName" value={foodItem.foodItemName} onChange={handleInputChange} />*/}
              ...
              <TableCell>
              <form onSubmit={submitOrder}>
          <TextField focused size="small"   style = {{width: 100}} inputProps={{ inputMode: 'numeric' , readOnly: true}} label="Quantity" name="quantity" id="quantity" value={foodItem.quantity}
           onChange={handleInputChange} />
</form>
       </TableCell>
         ...
         <TableCell>
              <form onSubmit={submitOrder}>
          <TextField focused size="small"   style = {{width: 100}} label="HUID" name="huid" id="huid" value={foodItem.huid}
           onChange={handleInputChange} />
</form>
       </TableCell>
        <TableCell>
              <form onSubmit={submitOrder}>
          <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Weight" name="foodItemPrice" id="foodItemPrice" value={foodItem.foodItemPrice}
           onChange={handleInputChange} />
</form>
       </TableCell>
