...
useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.SHOPBILLSILVER).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.SHOPBILLSILVER).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.SHOPBILLSILVER).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
