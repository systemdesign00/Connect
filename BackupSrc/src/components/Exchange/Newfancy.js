...
export default function Newfancy(props, initialFValues) {
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && orderedoldcashitems.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, orderedoldcashitems])
   let today = new Date();
...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        ...
        setValues({
            ...values,
            oldcashitems: [...values.oldcashitems, x]
        })
...
const wordify = (num) => {
 ...
 <form onSubmit={submitOrder}>
          <TextField focused fullWidth label="Item Name" name="foodItemName" id="foodItemName" value={foodItem.foodItemName} onChange={handleInputChange} />
</form>
   </TableCell>
 
        <TableCell>
            <form onSubmit={submitOrder}>
          <TextField focused fullWidth inputProps={{ inputMode: 'numeric' }} label="Cash" name="foodItemPrice" id="foodItemPrice" value={foodItem.foodItemPrice} onChange={handleInputChange} />
</form>
