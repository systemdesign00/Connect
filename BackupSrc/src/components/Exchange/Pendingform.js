...
values.cashitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0);
        let gTotal = newitemtotal - (values.oldorderDetails.reduce((total, currentValue) => total = total + currentValue.subtotals,0) + Number(values.oldcashitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0)));
...
// eslint-disable-next-line react-hooks/exhaustive-deps
    }, [JSON.stringify(values.orderDetails),JSON.stringify(values.oldorderDetails),JSON.stringify(values.cashitems),JSON.stringify(values.oldcashitems)]);
...
*/
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.EXCHANGE).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.EXCHANGE).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.EXCHANGE).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
...
//values.shopName = searchSelectedCountry &&   searchSelectedCountry.shopName 
