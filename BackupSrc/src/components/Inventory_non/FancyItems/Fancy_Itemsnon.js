...
export default function Fancy_Itemsnon() {
...
useEffect(() => {
        createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).fetchAll()
            .then(res => {
...
const navigate = useNavigate();
  const Fetchuser=()=>{
      createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primaryfetchAllfancyitems()
  ...
  const Createuser=async()=>{
      createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primarycreateitem(values)
  ...
  const Updateuser=async()=>{
     createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primaryupdatefancyitem(values.id,values)
...
const [orderListVisibilityFull, setOrderListVisibilityFull] = useState(false);
 ...
 const onDelete = id => {
        setConfirmDialog(false)
       createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primarydeletefancyitem(id)
                   .then(res => {
                   Fetchuser();
                   FetchStk();
                resetForm()
...
const handleSubmit = e => {
        e.preventDefault()
        if (validate()) {
            createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primarycreatefancyitem(values)
