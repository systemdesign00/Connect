...
export default function Silver_Itemsnon() {
...
const navigate = useNavigate();
  ...
  const Createuser=async()=>{
      createAPIEndpoint(ENDPIONTS.SDSILVERSTOCK).primarycreateitem(values)
  ...
  const Updateuser=async()=>{
     createAPIEndpoint(ENDPIONTS.SDSILVERSTOCK).primaryupdatesilveritem(values.id,values)
...
const [orderListVisibilityFull, setOrderListVisibilityFull] = useState(false);
 ...
 const onDelete = id => {
        setConfirmDialog(false)
       createAPIEndpoint(ENDPIONTS.SDSILVERSTOCK).primarydeletesilveritem(id)
                   .then(res => {
                   Fetchuser();
                   FetchStk();
                resetForm()
...
const handleSubmit = e => {
        e.preventDefault()
        if (validate()) {
            createAPIEndpoint(ENDPIONTS.SDSILVERSTOCK).primarycreatesilveritem(values)
        ...
        <TableBody>
          {datas().map(users => (
            ...
            <TableCell > <Box fontWeight="fontWeightBold" fontSize={16}>{new Date(users.hireDate).toLocaleDateString()}</Box></TableCell>
              ...
              <TableCell >  <Box fontWeight="fontWeightBold" fontSize={16}>{users.HUID}</Box></TableCell>
              ...
              <TableCell>  <Box fontWeight="fontWeightBold" fontSize={16}>{users.itemType}</Box></TableCell>
               <TableCell >  <Box fontWeight="fontWeightBold" fontSize={16}>{users.itemSize}</Box></TableCell>
