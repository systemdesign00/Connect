...
const navigate = useNavigate();
  const Fetchuser=()=>{
      createAPIEndpoint(ENDPIONTS.GSTGOLDSTOCK).primaryfetchAllitems()
...
const resetImg = () =>{
  ...
  const Createuser=async()=>{
      createAPIEndpoint(ENDPIONTS.GSTGOLDSTOCK).primarycreateitem(values)
  ...
  const Updateuser=async()=>{
     createAPIEndpoint(ENDPIONTS.GSTGOLDSTOCK).primaryupdateitem(values.id,values)
...
const [deleting, setDeleting] = useState(false);
 const onDelete = async (id) => {
       ...
       createAPIEndpoint(ENDPIONTS.GSTGOLDSTOCK).primarydeleteitem(id)
                   .then(res => {
                   FetchStk()
                   Fetchuser();
                resetForm()
             ...
             if (itemIndex !== -1) {
              const newId = generateNewId();
               ...
               try {
                const response =  axios.post('https://serdb.onrender.com/api/DelInventory', deletedItem);
  ...
  if (deleting) {
    intervalId = setInterval(async () => {
      ...
      try {
        ...
        await axios.delete(`https://serdb.onrender.com/api/GstGoldStock/${idToDelete}`);
        ...
        console.log('Posted modified deleted item to DelInventory:', deletedItem);
        ...
        function: 
         <MDBox ml={-1}>
            <MDBadge  badgeContent={
                  <Job title={highlightSearchTerm(item.HUID)} />
...
<ThemeProvider theme={blueTheme}>
 <Button className={`${styles.roots}`} style={{ backgroundColor: "#ABD1FF"}}  onClick={() => EditOrRemove(item)}>
  ...
  <MDTypography variant="h6" color="white">
                  ...
                  NET WEIGHT ➼➼  {roundTo2DecimalPoint(goldstk).toFixed(3)}G 
                </MDTypography>
                       </MDBox>
                  </MDBox>
