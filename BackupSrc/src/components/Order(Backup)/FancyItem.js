  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //  fetch data
        const dataUrl = `https://serdb.onrender.com/api/FancyStockitem`;
...
const deleteUserDatafancy = async (id) => {
   ...
   setConfirmDialog({
                      ...
                      subTitle: "You can't undo this operation",
                        onConfirm: () => { deleteUserDatafancy(user.id); }
                    });
    } 
  const searchSelectedCountry = countries.find((user) => {
    if (user.orderNumber === selectedCountry) {
      if (window.confirm('Are you sure to Add this Item ?')) {
       return true,deleteUserDatafancy(user.id);
   }
