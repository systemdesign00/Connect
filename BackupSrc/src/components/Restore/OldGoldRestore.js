...
export default function OldGoldRestore() {
...
const handleCheckboxChange = (id) => {
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios.delete(`https://serdb.onrender.com/api/Deloldgoldbill/${idToDelete}`)
            .then(response => {
            ...
            if (itemIndex !== -1) {
              ...
              try {
                const response =  axios.post('https://serdb.onrender.com/api/Oldgoldpurchase', deletedItem);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'https://serdb.onrender.com/api/Deloldgoldbill'; // Replace with your API endpoint
