...
export default function OldSilverRestore() {
...
const handleCheckboxChange = (id) => {
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios.delete(`https://serdb.onrender.com/api/Deloldsilverbill/${idToDelete}`)
            .then(response => {
            ...
            if (itemIndex !== -1) {
              ...
              try {
                const response =  axios.post('https://serdb.onrender.com/api/Oldsilverpurchase', deletedItem);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'https://serdb.onrender.com/api/Deloldsilverbill'; // Replace with your API endpoint
