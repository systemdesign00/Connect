...
export default function SilverRestore() {
...
const handleCheckboxChange = (id) => {
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios.delete(`https://serdb.onrender.com/api/Delsilvergstbill/${idToDelete}`)
            .then(response => {
            ...
            if (itemIndex !== -1) {
              ...
              try {
                const response =  axios.post('https://serdb.onrender.com/api/QSilverGST', deletedItem);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'https://serdb.onrender.com/api/Delsilvergstbill'; // Replace with your API endpoint
...
label= {deleting ? 'Deleting...' : 'Delete'} variant="outlined"  disabled={deleting}/>
  ...
  <Grid item xs={6}>
      ...
      MOBILE:{Data.mobile}
       ...
       <Typography variant="subtitle1" gutterBottom sx={{fontWeight: 'bold',color:'black'}}>
     {/*  PAN NO:{Data.pannumber} */} AADHAAR NO: {Data.aadhaarnumber}
      </Typography>
      <Typography variant="subtitle1" gutterBottom sx={{fontWeight: 'bold',color:'black'}}>
