...
export default function SilverVoucherRestore() {
...
const handleCheckboxChange = (id) => {
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios.delete(`https://serdb.onrender.com/api/Delsilverworkvoucherbill/${idToDelete}`)
            .then(response => {
            ...
            if (itemIndex !== -1) {
              ...
              try {
                const response =  axios.post('https://serdb.onrender.com/api/SilverVoucher', deletedItem);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'https://serdb.onrender.com/api/Delsilverworkvoucherbill'; // Replace with your API endpoint
