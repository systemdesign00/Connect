...
export default function Sterlinginventoryrestore() {
...
const handleCheckboxChange = (id) => {
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios.delete(`https://serdb.onrender.com/api/DelSterlingstk/${idToDelete}`)
            .then(response => {
   ...
   const formattedDate = currentDate.toLocaleDateString('en-IN', {
    ...
    year: 'numeric',
            ...
            if (itemIndex !== -1) {
              ...
              try {
                const apiUrl = 'https://serdb.onrender.com/api/FancyStockitem'; // Replace with your API endpoint
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const apiUrl = 'https://serdb.onrender.com/api/DelSterlingstk'; // Replace with your API endpoint
      ...
      <TableContainer sx={{ maxHeight: 440 }}>
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableRow>
            ...
            <StyledTableCell><Box fontWeight="fontWeightBold" fontSize={15}>ITEM ID</Box></StyledTableCell>
            <StyledTableCell ><Box fontWeight="fontWeightBold" fontSize={15}>DATE</Box></StyledTableCell>
         {/*   <StyledTableCell ><Box fontWeight="fontWeightBold" fontSize={15}>HUID</Box></StyledTableCell>*/}
            <StyledTableCell ><Box fontWeight="fontWeightBold" fontSize={15}>ITEM NAME</Box></StyledTableCell>
            <StyledTableCell ><Box fontWeight="fontWeightBold" fontSize={15}>ITEM WEIGHT</Box></StyledTableCell>
            ...
            {apiData
              ...
              .map((row) => {
                return (
                 ...
                 <StyledTableCell > <Box fontWeight="fontWeightBold" fontSize={15}>{row.orderNumber}</Box> </StyledTableCell>
                <StyledTableCell > <Box fontWeight="fontWeightBold" fontSize={15}>{new Date(row.hireDate).toLocaleDateString()}</Box></StyledTableCell>
             {/*    <StyledTableCell> <Box fontWeight="fontWeightBold" fontSize={15}>{row.HUID}</Box></StyledTableCell> */}
                 <StyledTableCell > <Box fontWeight="fontWeightBold" fontSize={15}>{row.itemName}</Box></StyledTableCell>
                 <StyledTableCell> <Box fontWeight="fontWeightBold" fontSize={15}>{row.itemWeight}G</Box></StyledTableCell>
