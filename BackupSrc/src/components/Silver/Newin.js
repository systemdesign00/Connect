...
let gsttotalshalf = Number(newitemtotals + makingcal) - values.gCash;
let gsttotals = Number(((Math.round(gsttotalshalf) * 1.5)/100));

let scGst = Math.round(Number(gsttotals + gsttotals));
let gstcal= ( makingcal ) 
values.scGst =  Math.round((((Number(newitemtotals) + Number(makingcal)) ) + Number(scGst)) - Number(values.gCash)) 
...
useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.STANDARDGSTSILVER).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.STANDARDGSTSILVER).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.STANDARDGSTSILVER).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
