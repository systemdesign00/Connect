...
import sin from '../../img/estimate.png'
...
const errorsound = new Audio('https://www.myinstants.com/media/sounds/erro.mp3')
  ...
  const [filterFn, setFilterFn] = useState({ fn: items => { return items; } })
    ...
    const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, title: '', subTitle: '' })
     const fetchAlled = () => {
         createAPIEndpoint(ENDPIONTS.SILVERESTIMATE).fetchAll()
            .then(res => {
                setData(res.data)
            })
...
const [query, setQuery] = React.useState('idle');
  ...
  const EditOrRemove=(item)=>{
   ...
   handleClickOpen()
       ...
       createAPIEndpoint(ENDPIONTS.SILVERESTIMATE).delete(id)
                   .then(res => {
                    createAPIEndpoint(ENDPIONTS.SILVERESTIMATE).fetchAll()
                    .then(res => {
     ...
     <Button className={`${classes.roots}`} style={{ backgroundColor: "#cdffcd"}} 
      ...
      >
         <DialogTitle className={classes.dialogTitle} style={{ cursor: 'move' }} id="draggable-dialog-title">
            ...
            >
              {query !== 'idle'}
             <Pdf targetRef={componentRef} filename={"GSTEstimateReport[Silver].pdf"}>
