...
const tday = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const oldaddFoodItem = oldfoodItem => {
        let x = {
            ...
            subtotals:(oldfoodItem.foodItemPrices)  * b || Math.round(total)
        }
        setValues({
            ...values,
 ...
 const submitOrder = e => {
        e.preventDefault();
           ...
           oldaddFoodItem(oldfoodItem)
                createAPIEndpoint(ENDPIONTS.SALESOLDSILVERREPORTSILVER).salesoldsilvercreate(oldfoodItem)
                    .then(res => {
                      //audio.play()
                       //resetFormControls();
                      // resetForm()
...
oldfoodItem.rate = 1
//oldfoodItem.foodItemNames = "Cash"
...
<TableContainer component={Paper}>
 ...
 <TextField focused size="small" style = {{width: 200}}inputProps={{ inputMode: 'numeric' }} label="Cash" name="foodItemNames" id="foodItemNames" value={oldfoodItem.foodItemNames} onChange={handleInputChange} />
  </form>
   </TableCell>
        <TableCell>
            <form onSubmit={submitOrder}>
              <TextField focused size="small" style = {{width: 100}}inputProps={{ inputMode: 'numeric' }} label="Amount" name="foodItemPrices" id="foodItemPrices" value={oldfoodItem.foodItemPrices} onChange={handleInputChange} />
 </form>
       </TableCell>
       {/* <TableCell>
            <form onSubmit={submitOrder}>
