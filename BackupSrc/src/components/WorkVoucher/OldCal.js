...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const oldaddFoodItem = oldfoodItem => {
        let x = {
            ...
            subtotals:total || (oldfoodItem.foodItemPrices * oldfoodItem.quantitys)/100
        }
        setValues({
            ...values,
...
const rates =   display.map(item => (item.rate))
  oldfoodItem.foodItemNames = "GOLD"
...
oldfoodItem.subtotals = total;
let perconv = oldfoodItem.taxs/100;
let copper = (Number(total)/(perconv)).toFixed(3) 
let grosswt = (copper - total).toFixed(3)
    return (
        <>
...
<TextField focused size="small" style = {{width: 200}} label="ItemName" name="foodItemNames" id="foodItemNames" value={oldfoodItem.foodItemNames} onChange={handleInputChange} />
   ...
   {/*<TableCell>
        ...
        <TableCell>
             <form onSubmit={submitOrder}>
           <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Purity" name="quantitys" id="quantitys" value={oldfoodItem.quantitys} onChange={handleInputChange} />
</form>
 </TableCell>
  ...
  <TableCell>
     ...
     <TextField focused size="small"  InputProps={{
          ...
          readOnly:'true'
        }} style = {{width: 150}} label="Net Pure" name="subtotals" id="subtotals" value={Number(total).toFixed(3) } onChange={handleInputChange} />
             </form>
             </TableCell> 
              <TableCell>
             <form onSubmit={submitOrder}>
           <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric',readOnly:'true' }} label="Cu/Au" name="taxs" id="taxs" value={oldfoodItem.taxs+"%"} onChange={handleInputChange} />
</form>
 </TableCell>

  <TableCell>
             <form onSubmit={submitOrder}>
           <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Purity"  value={copper}  />
</form>
 </TableCell>

  <TableCell>
             <form onSubmit={submitOrder}>
           <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Net Weight"  value={grosswt} onChange={handleInputChange} />
</form>
 </TableCell>
{/*   <TableCell>
        <form onSubmit={submitOrder}>
