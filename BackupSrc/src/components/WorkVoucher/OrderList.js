...
const [deleting, setDeleting] = useState(false);
  ...
  const [filterFn, setFilterFn] = useState({ fn: items => { return items; } })
    ...
    datas,
    TblHead,
} = useTable(data, filterFn,userService.headCellsworkvoucher);
const handleSearchcustomer = e => {
 ...
 x.billNo.toLowerCase().includes(target.value) || x.city.toLowerCase().includes(target.value) ||
...
const openEqualizepg = () => {
  ...
  var lastFive = iddate.substr(iddate.length - 4);
    var nextyear = Number(lastFive) + Number(1);
       ...
       createAPIEndpoint(ENDPIONTS.WORKVOUCHER).delete(id)
                   .then(res => {
                   createAPIEndpoint(ENDPIONTS.WORKVOUCHER).fetchAll()
            .then(res => {
              setData(res.data);
             ...
             .catch(err => console.log(err))
               ...
               fetchAlled();
                     try {
                      const response =  axios.post('https://serdb.onrender.com/api/Delgoldworkvoucherbill', deletedItem);
...
useEffect(() => {
  ...
  axios.get('https://serdb.onrender.com/api/WorkVoucher')
  ...
  if (deleting) {
    intervalId = setInterval(() => {
      if (selectedIds.length === 0) {
        ...
        axios.delete(`https://serdb.onrender.com/api/WorkVoucher/${idToDelete}`)
          .then(response => {
                    ...
                    <Checkbox 
                        ...
                        />
                          ...
                          {/*    <TableCell>Status</TableCell> */}
                            ...
                            <TableCell>NetPurity Weight</TableCell>
    ...
    year: 'numeric',
            ...
            icon={<Check fontSize="small" />}/>
              ...
              <TableCell
                        ...
                        {
                        <Box fontWeight="fontWeightBold" fontSize={15}>
                        {  item.oldorderDetails.reduce((total, currentValue) => total = total + Number(currentValue.foodItemPrices),0)}G
                    </Box>
}
...
<Popup
      ...
      >
         <DialogTitle className={classes.dialogTitle} style={{ cursor: 'move' }} id="draggable-dialog-title">
            ...
            >
              {query !== 'idle'}
             <Pdf targetRef={componentRef} filename={"WORKVOUCHER.pdf"}>
