...
// eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.OLDGOLDPURCHASE).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.OLDGOLDPURCHASE).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.OLDGOLDPURCHASE).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
