  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //  fetch data
        const dataUrl = index.BASE_URL+`SilverItems`;
...
const deleteUserDatasilver = async (id) => {
  ...
  const searchSelectedCountry = countries.find((user) => {
    if (user.orderNumber === selectedCountry) {
      if (window.confirm('Are you sure to Add this Item ,Item cannot be add agin contact admin?')) {
       return true,deleteUserDatasilver(user.id);
   }
