...
export default function Cash(props, initialFValuescash) {
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && ordercashitems.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, ordercashitems])
   let today = new Date();
...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const [foodItem, setData] = useState(initialFValuescash)
    function handle(e) {
...
const submitOrder = e => {
        e.preventDefault();
           ...
           addFoodItem(foodItem)
                createAPIEndpoint(ENDPIONTS.SALESREPORT).salespcscreate(foodItem)
                    .then(res => {
                      //audio.play()
                       //resetFormControls();
                      // resetForm()
