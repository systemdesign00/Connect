...
const blackTheme = createTheme({ palette: { primary: grey } })
...
export default function Pendinglist(props,initialFValues,initialFValuesilver,initialFValuesold,initialFValueswastage,initialFValueswastagecash,initialFValuesfancy,initialFValuespiece) {
...
const { values, setValues,ops } = props;
    ...
    let orderedFoodItemsilver = values.orderDetailsilver;
    ...
    const removeFoodItem = (index, id) => {
     ...
     const removeFoodItemsilver = (index, id) => {
        //debugger;
        let x = { ...values };
        x.orderDetailsilver = x.orderDetailsilver.filter((_, i) => i !== index);
        if (id !== 0)
            x.deletedOrderItemIds += id + ',';
        setValues({ ...x });
    }
 ...
 foodItem.subtotal = newval === data.type ?  (data.foodItemPrice * data.rate) + Number(data.tax) : Number(((data.foodItemPrice - data.lessweight) * data.tax ) / 100) * data.rate;
      ...
      const updatesilver = (idx) => {
        ...
        let foodItem = x.orderDetailsilver[idx];
        foodItem.foodItemName = data.foodItemName
           foodItem.foodItemPrice = data.foodItemPrice
              foodItem.quantity = data.quantity
              foodItem.tax = data.tax
...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
 ...
 initialFValuesilver = {
  ...
  function handlesilver(e) {
        const newdatasilver = { ...datasilver }
        newdatasilver[e.target.id] = e.target.value
        setDatasilver(newdatasilver)
}
...
const [isEditingwastagecash, setIsEditingwastagecash] = useState(false)
 const [currentIdwastagecash, setCurrentIdwastagecash] = useState(0)
  ...
  useEffect(() => {
    ...
    }, [currentId,currentIdold,currentIdfancy,currentIdwastage,currentIdwastagecash,currentIdsilver])
...
*/}
  ...
  var resultwastage =  values.watageitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0);
   ...
   var resultsilver =  values.orderDetailsilver.reduce((total, currentValue) => total = total + currentValue.subtotal,0);
  ...
  var newitem = values.orderDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0) + resultwastage  + resultpcs + resultsilver ;
 var totalfiness = values.orderDetails.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0) + 
 values.orderDetailsilver.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0)
                                            ...
                                            <>
{
 ...
 {  isEditingsilver ===idx ? newval === data.type ?  Math.round((datasilver.foodItemPrice * datasilver.rate) + Number(datasilver.tax)) : NumberFormat(Math.round(Number(((datasilver.foodItemPrice - datasilver.lessweight) * datasilver.tax ) / 100) * datasilver.rate))
             :   <Div > { NumberFormat(Math.round((item.subtotal)))}</Div>
                                         }
                                       </TableCell>
                ...
                :
                                         ...
                                         onClick={()=>{
                                            ...
                                            <ThemeProvider theme={redTheme}>
                                                <Button className={`${classes.root}`} style={{  backgroundColor: '#ffbfbf'}} 
                                                onClick={e => removeFoodItemsilver(idx, item.orderDetailIdsilver)}
                                                > <DeleteSweepOutlinedIcon  fontSize="medium" />
                                                </Button>
                                            </ThemeProvider>
                                        </TableCell>
                                            ...
                                            <>
{
  ...
  <TableContainer >
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table" className={classes.tablebill}>
        <TableHead>
          <TableRow>
           <TableCell>{values.ventorName}</TableCell>
           <TableCell ></TableCell>
            <TableCell ></TableCell>
            <TableCell ></TableCell>
             <TableCell ></TableCell>
