...
let newitemtotal = values.orderDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0) +  values.watageitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0) + 
 ...
 let  Equity = Number(values.oldorderDetails.reduce((total, currentValue) => total = total + currentValue.subtotals,0) + values.fancyitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0)) +
        ...
        let gTotal = values.orderDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0) +  values.orderDetailsilver.reduce((total, currentValue) => total = total + currentValue.subtotal,0) +  values.watageitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0) + 
 values.pieceitems.reduce((total, currentValue) => total = total + currentValue.subtotal,0); 
        // let gTotal = Number(newitemtotal) - Number(Equity)
         let purchasesilver = 0; 
  let purchase =  values.orderDetails.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0) + values.orderDetailsilver.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0); ; 
   ...
   let adjustfiness = values.oldorderDetails.reduce((total, currentValue) => total = total + Number(currentValue.Tfinesss),0) + Number(cashfiness) + Number(purchasesilver)
   let netbalance = (Number(adjustfiness) - Number(purchase)).toFixed(3)
   setValues({
            ...values,
            gTotal: Math.round(gTotal),
            purchasewt:Number(purchase + purchasesilver).toFixed(3),
            pannumber:Number(adjustfiness).toFixed(3),
            balance:Number(netbalance).toFixed(3)
        }) 
// eslint-disable-next-line react-hooks/exhaustive-deps
    }, [JSON.stringify(values.orderDetails),JSON.stringify(values.orderDetailsilver),JSON.stringify(values.oldorderDetails),JSON.stringify(values.watageitems),JSON.stringify(values.fancyitems),JSON.stringify(values.pieceitems)]);
...
*/
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.PURCHASESILVER).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
/*const validateForm = (fieldValues = values) => {
 ...
 //temp.watageitems = fieldValues.watageitems.length !== 0 ? "" : "This field is required.";
  ...
  /*
    ...
    const validateForm = () => {
        let temp = {};
        temp.ventorName = values.ventorName != 0 ? "" : "This field is required!"
...
const getFreshModelObject = () => {
                         values.ventorName=''
                           values.mobile=''
                             values.city=''
                          searchSelectedCountry.ventorName=""
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.PURCHASESILVER).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
...
values.ventorName = searchSelectedCountry &&   searchSelectedCountry.ventorName || values.ventorName 
...
values.ventorId = searchSelectedCountry && searchSelectedCountry.ventorId || values.ventorId
values.city = searchSelectedCountry &&searchSelectedCountry.city ||  values.city
  ...
  <Grid item xs={6}>
   <FormControl size="small" fullWidth>
          <InputLabel id="search-select-label">Select Ventor</InputLabel>
        <Select
