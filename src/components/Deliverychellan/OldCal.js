...
const Todaysales = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const oldaddFoodItem = oldfoodItem => {
        let x = {
            ...
            subtotals: Math.round(total) || (oldfoodItem.foodItemPrices) * oldfoodItem.rate
        }
        setValues({
            ...values,
...
<TextField focused size="small" style = {{width: 200}} label="ItemName" name="foodItemNames" id="foodItemNames" value={oldfoodItem.foodItemNames} onChange={handleInputChange} />
   ...
   {/*<TableCell>
        ...
        <TableCell>
             <form onSubmit={submitOrder}>
           <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="UOM" name="quantitys" id="quantitys" value={oldfoodItem.UOM} onChange={handleInputChange} />
</form>
 </TableCell>
   <TableCell>
        <form onSubmit={submitOrder}>
