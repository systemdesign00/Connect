...
// eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.SHOPBILL).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.SHOPBILL).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.SHOPBILL).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
