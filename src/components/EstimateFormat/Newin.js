...
import { AccountBalance, AccountCircleOutlined, MobileFriendlyOutlined, PlaceOutlined } from '@mui/icons-material';
...
let oldsilver = values.oldorderDetailsilver.reduce((total, currentValue) => total = total + currentValue.subtotals,0)
        let gTotal = newitemtotal - (values.oldorderDetails.reduce((total, currentValue) => total = total + currentValue.subtotals,0) + Number(oldsilver))
...
// eslint-disable-next-line react-hooks/exhaustive-deps
    }, [JSON.stringify(values.orderDetails),JSON.stringify(values.orderPercent),JSON.stringify(values.oldorderDetails),JSON.stringify(values.oldorderDetailsilver),JSON.stringify(values.watageitems),JSON.stringify(values.addorderDetails),JSON.stringify(values.stockaddorderDetails),JSON.stringify(values.fancyitems),JSON.stringify(values.pieceitems)]);
...
*/
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.GSESTIMATE).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
<Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
  <Grid item xs={4}>
    {
      values.gTotal < 0 ? <TextField disabled
                style ={{width: '90%'}}
                size="small"
                            label="Grand Total"
                            name="gTotal"
