...
import {InputAdornment , Container,Button, TextField, IconButton ,Typography,Toolbar,Checkbox} from '@mui/material';
...
const [deleting, setDeleting] = useState(false);
 const onDelete = async (id) => {
        setConfirmDialog(false)
       createAPIEndpoint(ENDPIONTS.FANCYITEMS).primarydeleteitem(id)
                   .then(res => {
                   FetchStk()
                   Fetchuser();
                resetForm()
    ...
    year: 'numeric',
             ...
             if (itemIndex !== -1) {
              ...
              const updatedData = [...data.slice(0, itemIndex), ...data.slice(itemIndex + 1)];
               ...
               try {
                const response =  axios.post('https://serdb.onrender.com/api/DelSterlingstk', deletedItem);
  ...
  if (deleting) {
    intervalId = setInterval(async () => {
      ...
      try {
        ...
        await axios.delete(`https://serdb.onrender.com/api/FancyStockitem/${idToDelete}`);
...
const deletedItem = {
        ...
        await axios.post('https://serdb.onrender.com/api/DelSterlingstk', deletedItem);
...
itemType:item.itemType
  ...
  const fileName = 'Sterlingsilver.xlsx';
...
const handleExport = () => {
  if (data.length > 0) {
    ...
    // Save the Blob as a file
    const fileName = 'SterlingInventory.xlsx';
    saveAs(blob, fileName);
  } else {
    // If no data, export only the header
    const header = ['hireDate', 'HUID', 'itemName', 'itemWeight', 'itemPrice', 'itemModel', 'itemType'];
...
const reset=()=>{
 ...
 var stk =0
var sterlingstk = displaysilverstk.reduce((total, currentValue) => total = total + Number(currentValue.itemWeight),0);
...
<ThemeProvider theme={blueTheme}>
 ...
 <DashboardLayout>
  ...
  <MDTypography variant="h6" color="white" onClick={handleExport}>
                STERLING INVENTORY MANAGEMENT
                </MDTypography>

                  <MDTypography variant="h6" color="black">
                  NET WEIGHT ➼➼ {roundTo2DecimalPoint(sterlingstk).toFixed(3)}G 
                </MDTypography>
                       </MDBox>
                  </MDBox>
      ...
      <Avatar  src={values.imageSrc} sx={{ width: 50, height: 50 ,bgcolor: 'white'}}>
        ...
        <IconButton color="info" aria-label="upload picture" component="span">
          <PhotoCamera />
            ...
            <Grid item xs={6}>
                <Autocomplete
     options={userService.SterlingType}
     freeSolo
     fullWidth
     onChange={handleChange}
