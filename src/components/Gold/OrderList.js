  ...
  const handleClickQuery = () => {
    ...
    timerRef.current = window.setTimeout(() => {
      setQuery('success');
       ...
       createAPIEndpoint(ENDPIONTS.STANDARDGST).delete(id)
                   .then(res => {
                   createAPIEndpoint(ENDPIONTS.STANDARDGST).fetchAll()
            .then(res => {
      ...
      >
       ...
       : 
        ...
        {item.hireDate}
          ...
          item.stockaddorderDetails.reduce((total, currentValue) => total = total + Number(currentValue.foodItemPrice),0)}G
                                    </TableCell>
