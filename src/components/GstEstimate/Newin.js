...
// eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.GSTESTIMATE).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
values.scGst = Math.round(((values.gTotal + makingcal + (Number(hallmarkingcal)) ) + (gstpertotal + gstpertotal)) - Number(values.gCash));
let oldcal =  oldorderedFoodItems.reduce((total, currentValue) => total = total + Number(currentValue.subtotals),0);
values.status = NumberFormat(Number(oldcal))
...
const getFreshModelObject = () => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.GSTESTIMATE).create(values)
                    .then(res => {
                      audio.play()
                       resetFormControls();
                ...
                createAPIEndpoint(ENDPIONTS.GSTESTIMATE).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
