...
const navigate = useNavigate();
  const Fetchuser=()=>{
      createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primaryfetchAllsilveritems()
...
const handleSubmit = e => {
        e.preventDefault()
        if (validate()) {
            createAPIEndpoint(ENDPIONTS.SDFANCYSTOCK).primarycreatesilveritem(values)
      ...
      <Table className={styles.table}>
        ...
        <TableBody>
          ...
          <TableCell><Box fontWeight="fontWeightBold" fontSize={16}>{users.itemName}</Box></TableCell>
           <TableCell></TableCell>
              <TableCell ><Box fontWeight="fontWeightBold" fontSize={16}>{users.occurrence}</Box></TableCell>
              </TableRow>
          ))}
        </TableBody>
      </Table>
