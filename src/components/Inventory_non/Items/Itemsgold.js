...
export default function Itemsgold() {
...
const navigate = useNavigate();
  const Fetchuser=()=>{
      createAPIEndpoint(ENDPIONTS.SDGOLDSTOCK).primaryfetchAllitems()
...
const resetImg = () =>{
  ...
  const Createuser=async()=>{
      createAPIEndpoint(ENDPIONTS.SDGOLDSTOCK).primarycreateitem(values)
  ...
  const Updateuser=async()=>{
     createAPIEndpoint(ENDPIONTS.SDGOLDSTOCK).primaryupdateitem(values.id,values)
...
const [orderListVisibilityFull, setOrderListVisibilityFull] = useState(false);
 ...
 const onDelete = id => {
       ...
       createAPIEndpoint(ENDPIONTS.SDGOLDSTOCK).primarydeleteitem(id)
                   .then(res => {
                   FetchStk()
                   Fetchuser();
                resetForm()
