...
const openEqualizepg = () => {
  ...
  return Math.floor(Math.random() * 1000);
       ...
       createAPIEndpoint(ENDPIONTS.OLDSILVERPURCHASE).delete(id)
                   .then(res => {
                   createAPIEndpoint(ENDPIONTS.OLDSILVERPURCHASE).fetchAll()
            .then(res => {
              setData(res.data);
             ...
             .catch(err => console.log(err))
               ...
               fetchAlled();
                     try {
                      const response =  axios.post('https://serdb.onrender.com/api/Deloldsilverbill', deletedItem);
...
useEffect(() => {
  ...
  axios.get('https://serdb.onrender.com/api/Oldsilverpurchase')
  ...
  if (deleting) {
    intervalId = setInterval(() => {
      if (selectedIds.length === 0) {
        ...
        axios.delete(`https://serdb.onrender.com/api/Oldsilverpurchase/${idToDelete}`)
          .then(response => {
