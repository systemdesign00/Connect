...
const addremoveFoodItem = (index, id) => {
 ...
 foodItem.subtotal = newval === data.type ?  (data.foodItemPrice * data.rate) + Number(data.tax) : (data.foodItemPrice * data.quantity * rates) - (data.discount * data.foodItemPrice * data.quantity * rates) / 100 + data.tax * (data.foodItemPrice * data.quantity * rates) / 100 
        setValues({...x});
        setIsEditing(false)
        //resetInputField()
      }
 ...
 foodItem.subtotal = itemwastages === datawastage.type ? ( Number(datawastage.foodItemPrice) + Number(datawastage.tax)) * rates : ( Number(datawastage.foodItemPrice) + Number(datawastage.tax )) * rates;
        setValues({...x});
 ...
 foodItem.subtotal = additem === dataadd.type ? ( Number(dataadd.foodItemPrice) * (Number(dataadd.tax)+ Number(rates)) ) :( Number(dataadd.foodItemPrice) * (Number(dataadd.tax)+ Number(rates)) );
        setValues({...x});
 ...
 foodItem.subtotal = stockadditem === stockdataadd.type ? ( Number(stockdataadd.foodItemPrice) * (Number(stockdataadd.tax)+ Number(rates)) ) :( Number(stockdataadd.foodItemPrice) * (Number(stockdataadd.tax)+ Number(rates)) );
        setValues({...x});
                                            ...
                                            <>
{
 ...
 { 
             ...
             isEditing ===idx ? newval === data.type ?  Math.round((data.foodItemPrice * data.rate) + Number(data.tax)) : Math.round((data.foodItemPrice * data.quantity * rates) - (data.discount * data.foodItemPrice * data.quantity * rates) / 100 + data.tax * (data.foodItemPrice * data.quantity * rates) / 100) 
                                            ...
                                            <>
{
       ...
       startAdornment:
         ...
         <TextField label="Rate" name="rate" id="rate" 
          ...
          <TableRow>
            <TableCell rowSpan={3} />
            <TableCell colSpan={5}>New_Items</TableCell>
 ...
 //NumberFormat((Math.round(Math.round(newitem) * (Number(values.cgst) + Number(values.sgst)) / 100 ) + Math.round((Number(values.cgst) + Number(values.sgst)) + newitem)))
          ...
          <TableRow>
            <TableCell colSpan={5}>Old_Items</TableCell>
            <TableCell > <Typography  display="block"  style={{color:'red'}}>
 {NumberFormat(Math.round(news).toFixed(2))}
      </Typography></TableCell>
            
          </TableRow>
          <TableRow>
               <TableCell colSpan={5}>Cash_Received</TableCell>
            <TableCell >  <Typography  display="block"  style={{color:'blue'}}>
 {NumberFormat(Number(values.gCash) + Number(values.onlinecash)) % 1 === 0
                                            ...
                                            <>
{
       ...
       startAdornment:
           ...
           inputProps={{ inputMode: 'numeric' }} label="quantity" name="quantitys" id="quantitys" 
           value={dataold.quantitys} onChange={(e) => handleold(e)} />
              ...
              <TableRow>
            <TableCell rowSpan={3} />
       <TableCell colSpan={2}>New_Item</TableCell>
            <TableCell >
 ...
 {NumberFormat(Math.round(newitem))}
          ...
          <TableRow>
            <TableCell colSpan={2}>Old_Items</TableCell>
            <TableCell >
               <Typography  display="block"  style={{color:'red'}}>
 {NumberFormat(oldresults.toFixed(2))}
      </Typography>
            </TableCell>
          ...
          <TableRow>
            <TableCell colSpan={2} >Cash_Received</TableCell>
            <TableCell >
 ...
 {NumberFormat(Number(values.gCash) + Number(values.onlinecash)) % 1 === 0
      ...
      <ItemPopup
        ...
        indicatorColor="secondary" >
          ...
          <Tab label="Stock_Decimal" {...a11yProps(1)} />
            <Tab label="Stock_AddNon_Stock" {...a11yProps(2)} />
          <Tab label="Non_Stock" {...a11yProps(3)} />
          <Tab label="Non_Stock_Decimal" {...a11yProps(4)} />
          <Tab label="AddNon_Stock" {...a11yProps(5)} />
        </Tabs>
      </Box>
      <TabPanel value={valuetabs} index={0}>
            <ConfirmProvider>
...
color: 'black',
fontWeight: "600"
  }}>{NumberFormat('-'+Math.round(oldresult))}</TableCell>
               <TableCell > </TableCell>
            </TableRow>
