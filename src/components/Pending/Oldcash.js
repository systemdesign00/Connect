...
export default function Oldcash(props, initialFValues) {
  ...
  const handleChangetab = (event, newValue) => {
    ...
    const { values, setValues, setOldListVisibilitycash,recordForEdit } = props;
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && ordercash.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, ordercash])
   let today = new Date();
...
const tday = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        ...
        setValues({
            ...values,
            ordercashDetails: [...values.ordercashDetails, x]
        })
        setOldListVisibilitycash(false);
        resetInputField()
    }
...
const total = (foodItem.foodItemPrice * foodItem.rate) + Number(foodItem.tax);
