...
export default function OrderList(props) {
      ...
      fn: (items) => {
        if (target.value === "") return items;
          ...
          return items.filter((x) =>
            x.shopName.toLowerCase().includes(target.value)
          );
      },
    });
  };
   ...
   const itemIndex = data.findIndex((item) => item.id === id);
      ...
      try {
        const response = axios.post(
          "https://serdb.onrender.com/api/Delwholesalebill",
          deletedItem
        );
  ...
  useEffect(() => {
    ...
    axios
      .get("https://serdb.onrender.com/api/Pending")
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/Pending/${idToDelete}`)
            .then((response) => {
              ...
              <TableRow key={item.id}>
                ...
                new Date(item.hireDate).toLocaleDateString() ? (
                  <TableCell onClick={(e) => showForUpdate(item.id)}>
                    ...
                    >
                      <Chip
                        label={new Date(item.hireDate).toLocaleDateString()}
