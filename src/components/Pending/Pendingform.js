...
export default function Pendingform(props) {
  ...
  useEffect(() => {
    createAPIEndpoint(ENDPIONTS.BOOKS)
      .fetchAll()
      .then((res) => {
        let customerList = res.data.map((item) => (item.id, item.fullName));
    ...
    createAPIEndpoint(ENDPIONTS.BOOKS)
      .fetchAll()
      .then((res) => {
        let customerListphone = res.data.map((item) => (item.id, item.mobile));
        //customerList = [{ id: 0, title: 'Select' }].concat(customerList);
  ...
  useEffect(() => {
    let newitemtotal =
      ...
      values.watageitems.reduce(
        (total, currentValue) => (total = total + currentValue.subtotal),
        0
      ) +
      values.pieceitems.reduce(
        (total, currentValue) => (total = total + currentValue.subtotal),
        0
      );
    ...
    let gTotal =
      values.ordercashDetails.reduce(
        (total, currentValue) => (total = total + currentValue.subtotal),
        0
      ) +
      Number(
    ...
    let cashfiness = values.fancyitems.reduce(
      (total, currentValue) =>
        (total =
          total + Number(currentValue.foodItemPrice * 1) / currentValue.rate),
      0
    ...
    JSON.stringify(values.oldorderDetails),
    JSON.stringify(values.watageitems),
    JSON.stringify(values.fancyitems),
    JSON.stringify(values.ratecutitems),
    JSON.stringify(values.pieceitems),
    JSON.stringify(values.ordercashDetails),
  ]);
...
*/
 ...
 //temp.watageitems = fieldValues.watageitems.length !== 0 ? "" : "This field is required.";
  ...
  const audio = new Audio(
    "https://www.winhistory.de/more/winstart/mp3/win11shutdown.mp3"
  );
  const audioupdate = new Audio(
    "https://www.winhistory.de/more/winstart/mp3/wfw311.mp3"
  );
  ...
  let Equity =
    ...
    values.ordersilverDetails.reduce(
      (total, currentValue) => (total = total + currentValue.subtotal),
      0
    );
  ...
  values.imageSrc =
    ...
    values.imageSrc;
  values.mobile =
    (searchSelectedCountry && searchSelectedCountry.mobile) || values.mobile;
  values.customerUID =
    (searchSelectedCountry && searchSelectedCountry.customerId) ||
    values.customerId;
  values.city =
    (searchSelectedCountry && searchSelectedCountry.city) || values.city;
  //values.pannumber = searchSelectedCountry &&searchSelectedCountry.pannumber  || values.pannumber
  values.aadhaarnumber =
    (searchSelectedCountry && searchSelectedCountry.aadhaarnumber) ||
    values.aadhaarnumber;
  values.Purebalance = (
    Number(values.purchasewt) +
    Number(values.cashreceivedonline) -
    (Number(values.pannumber) + Number(values.onlinecash))
  ).toFixed(3);
      ...
      */}
                ...
                <Grid item xs={4}>
                  <TextField
                    ...
                    label="Balance "
                    name="amountdebit"
                    value={NumberFormat(Number(values.gTotal) - Number(Equity))}
                    InputProps={{
                      startAdornment: (
                        <InputAdornment position="start">
                          <img src={Cashimg} height={25} />
