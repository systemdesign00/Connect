...
export default function Pendinglist(props,initialFValues,initialFValuesilver,initialFValuescash,initialFValuesold,initialFValueswastage,initialFValueswastagecash,initialFValuesfancy,initialFValuesratecut,initialFValuespiece) {
...
var oldcashbalance =  values.ordercashDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0);
  var newitem = values.orderDetails.reduce((total, currentValue) => total = total + currentValue.subtotal,0) + resultwastage  + resultpcs ;
 var totalfiness = values.orderDetails.reduce((total, currentValue) => total = total + Number(currentValue.Tfiness),0) + values.ratecutitems.reduce((total, currentValue) => total = total + Number(currentValue.foodItemPrice * 1)/currentValue.rate,0) + Number(values.cashreceivedonline);
       ...
       {/*   <TableCell sx={{ backgroundColor: "#c0c0c0"}} >Date</TableCell>*/}
        ...
        <TableRow >
         ...
         <TableCell ></TableCell>
            ...
            <TableCell sx={{ fontWeight:"bold"}}>{values.cashreceivedonline + 0.000}G</TableCell>
    ...
    {/* <TableCell > {new Date(item.salesdate).toLocaleDateString()}</TableCell>*/}
        ...
        <TableCell >{newval === item.type ? "-" : roundTo2DecimalPoint(Number(item.foodItemPrice  - item.lessweight)).toFixed(3)+"G"}</TableCell> */}
           ...
           {orderedFoodItemsilver.map((item,idx) => (
            <TableRow key={item.id}>
    {/* <TableCell > {new Date(item.salesdate).toLocaleDateString()}</TableCell>*/}
              <TableCell sx={{fontWeight:"bold"}}>{item.foodItemName+"-->"+"("+NumberFormat(Math.round((item.subtotal)))+"/"+item.rate+")"}</TableCell>
                {/*   <TableCell >{item.quantity}</TableCell> */}
              <TableCell >{item.foodItemPrice+"G"}</TableCell>
              {/* <TableCell>{newval === item.type ?  "-" : item.lessweight+"G"}</TableCell>
        <TableCell >{newval === item.type ? "-" : roundTo2DecimalPoint(Number(item.foodItemPrice  - item.lessweight)).toFixed(3)+"G"}</TableCell> */}
        ...
        {orderedratecutitems.map((item,idx) => (
            <TableRow key={item.id}>
     {/* <TableCell > {new Date(item.salesdate).toLocaleDateString()}</TableCell>*/}
          
              <TableCell sx={{fontWeight:"bold"}}>{item.foodItemName+"-->"+"("+item.foodItemPrice+"/"+item.rate+")"}</TableCell>
          ...
          {orderpieceitems.map((item,idx) => (
            <TableRow key={item.id}>
    {/* <TableCell > {new Date(item.salesdate).toLocaleDateString()}</TableCell>*/}
              <TableCell sx={{fontWeight:"bold"}}>{item.foodItemName+"-->"+"("+NumberFormat(item.foodItemPrice)+")"}</TableCell>
                 {/*  <TableCell >{item.quantity}</TableCell> */}
                    <TableCell >-</TableCell>
                   {/*   <TableCell >-</TableCell>
                        <TableCell >-</TableCell>*/}
...
<TableRow sx={{border: "none",
  ...
  boxShadow: "none"}} >Cash Bal:{NumberFormat(Number(cashbalance)+Number(orderpieceitemscash))}</TableCell>
  ...
  boxShadow: "none"}}>Net WT:</TableCell>
      ...
      <TableCell sx={{fontWeight:"bold"}} > {((totalfiness)).toFixed(3)}G</TableCell>
...
{oldorderedFoodItems.map((item,idx) => (
 ...
 {ordercash.map((item,idx) => (
  ...
  <TableCell sx={{fontWeight:"bold"}}>{datacash.foodItemName+"-->"+"("+NumberFormat(datacash.foodItemPrice)+")"}</TableCell>
      <TableCell >-</TableCell>
      <TableCell >-</TableCell>
     <TableCell >-</TableCell>
  ...
  boxShadow: "none"}}>
   ...
   <TableCell sx={{border: "none", fontWeight:"bold",
  boxShadow: "none"}} >Cash Bal:{NumberFormat(Number(oldcashbalance)+Number(oldorderpieceitemscash))}</TableCell>
  ...
  boxShadow: "none"}}>Net WT:</TableCell>
                   ...
                   <TableCell sx={{fontWeight:"bold"}}>{((adjustfiness)).toFixed(3)}G</TableCell> 
  ...
  boxShadow: "none"}}>
   ...
   <TableCell sx={{border: "none", fontWeight:"bold",
  boxShadow: "none"}} >Closing Cash Bal:{NumberFormat((Number(cashbalance)+Number(orderpieceitemscash)) - (Number(oldcashbalance)+Number(oldorderpieceitemscash)))}</TableCell>
 ...
 <TableCell sx={{fontWeight:"bold"}} >
  {/*(Number(totalfiness) - Number(adjustfiness)).toFixed(3)*/}
