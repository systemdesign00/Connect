...
export default function NonStockwastage(props, initialFValues) {

    const { values, setValues, setwastageListVisibility,recordForEdit } = props;
    ...
    const addFoodItems = foodItem => {
        let x = {
           ...
           salesdate:formattedDate,
            ...
            subtotal:(Number(foodItem.foodItemPrice) + (Number(foodItem.foodItemPrice) * Number(foodItem.tax))) * rates,
        }
        setValues({
            ...values,
            watageitems: [...values.watageitems, x]
        })
       // setwastageListVisibility(false);
        resetInputField()
    }
...
const a = Number(foodItem.foodItemPrice);
const wastcalc = Number(foodItem.foodItemPrice) * Number(foodItem.tax);
const awastcalc = Number(a) + Number(wastcalc);
//const taxRate = (foodItem.tax * a) / 100;
//const discountRate = (foodItem.discount * a) / 100;
//const total = a - discountRate + taxRate;
const total = awastcalc * rates;
const wordify = (num) => {
