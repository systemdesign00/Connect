...
const tday = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        let x = {
            ...
            orderDetailId: 0,
            type:'fancy',
              silvertypefancy:'stksilverfancy',
            model:'pcs',
            uom:foodItem.uom,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dataUrl = index.BASE_URL+`fancysilver`;
...
const [notify, setNotify] = useState({ isOpen: false })
  ...
  const validateForm = () => {
        ...
        //  temp.gCash = values.gCash != 0 ? "" : "This field is required.";
          temp.rate = foodItem.rate != 0 ? "" : "This field is required.";
...
const wordify = (num) => {
   ...
   <TableCell>
    ...
    options={userService.fancyornaments}
     freeSolo
...
/>
 ...
 <Button type="submit" variant="contained" size="small" onClick={submitOrder}  >Submit</Button>
  ...
  <Grid item >
  <Button  variant="contained" size="small" onClick={null} >Reset</Button>
  </Grid>
