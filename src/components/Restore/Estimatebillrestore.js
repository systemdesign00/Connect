...
const handleSearchweight = e => {
  ...
  useEffect(() => {
    createAPIEndpoint(ENDPIONTS.ESTIMATERESTORE)
      .fetchAll()
      .then((res) => {
    ...
    if (deletings) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/DelEstimatebill/${idToDelete}`)
            .then((response) => {
    ...
    if (deleting) {
      intervalId = setInterval(async () => {
        ...
        try {
          // Delete the first selected item from the API
          await axios.delete(
            `https://serdb.onrender.com/api/DelEstimatebill/${idToDelete}`
          );
          ...
          await axios.post("https://serdb.onrender.com/api/Gsestimate", deletedItem);
          console.log(
  ...
  const rows = data
    ...
    .map((item) => ({
      ...
      status: <ItemCompt description={highlightSearchTerm(item.fullName)} />,
