...
export default function Goldpurchasebill() {
...
const handleSearchweight = e => {
  ...
  useEffect(() => {
    createAPIEndpoint(ENDPIONTS.GOLDPURCHASERESTORE)
      .fetchAll()
      .then((res) => {
    ...
    if (deletings) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/DelGoldpurchase/${idToDelete}`)
            .then((response) => {
    ...
    if (deleting) {
      intervalId = setInterval(async () => {
        ...
        try {
          // Delete the first selected item from the API
          await axios.delete(
            `https://serdb.onrender.com/api/DelGoldpurchase/${idToDelete}`
          );
          ...
          await axios.post("https://serdb.onrender.com/api/Purchase", deletedItem);
          console.log(
