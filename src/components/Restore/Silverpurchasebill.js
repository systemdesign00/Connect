...
export default function Silverpurchasebill() {
...
const handleSearchweight = e => {
  ...
  useEffect(() => {
    createAPIEndpoint(ENDPIONTS.SILVERPURCHASERESTORE)
      .fetchAll()
      .then((res) => {
    ...
    if (deletings) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/DelSilverpurchase/${idToDelete}`)
            .then((response) => {
    ...
    if (deleting) {
      intervalId = setInterval(async () => {
        ...
        try {
          // Delete the first selected item from the API
          await axios.delete(
            `https://serdb.onrender.com/api/DelSilverpurchase/${idToDelete}`
          );
          ...
          await axios.post("https://serdb.onrender.com/api/Purchasesilver", deletedItem);
          console.log(
  ...
  const columns = [
    ...
    { Header: "MOBILE", accessor: "employed", align: "center" },
      { Header: "CITY", accessor: "employedcity", align: "center" },
    { Header: "STATUS", accessor: "weight", align: "center" },
  ];
  ...
  const rows = data
    ...
    .map((item) => ({
      ...
      status: <ItemCompt description={highlightSearchTerm(item.shopName)} />,
        
      employed: (
        <MDBox ml={-1}>
          <MDBadge
            badgeContent={<Job title={item.mobile} />}
            color="warning"
            size="sm"
          />
        </MDBox>
      ...
      employedcity: (
        <MDBox ml={-1}>
          <MDBadge
            badgeContent={<Job title={item.city} />}
            color="primary"
            size="sm"
          />
        </MDBox>
      ...
      weight: (
        <MDBox ml={-1}>
          <MDBadge
            badgeContent={
              <Job title={highlightSearchTerm(item.status)} />
            }
            color={item.staus === "Balance" ? "error" : "success"}
            size="sm"
          />
        </MDBox>
      ),
