...
const handleSearchweight = e => {
  ...
  useEffect(() => {
    createAPIEndpoint(ENDPIONTS.STERLINGRESTORE)
      .fetchAll()
      .then((res) => {
    ...
    if (deletings) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/DelSterlingstk/${idToDelete}`)
            .then((response) => {
    ...
    if (deleting) {
      intervalId = setInterval(async () => {
        ...
        try {
          // Delete the first selected item from the API
          await axios.delete(
            `https://serdb.onrender.com/api/DelSterlingstk/${idToDelete}`
          );
          ...
          await axios.post("https://serdb.onrender.com/api/FancyStockitem", deletedItem);
          console.log(
