...
import {InputAdornment , Container,Button, TextField, IconButton ,Typography,Checkbox,Toolbar} from '@mui/material';
...
import {useMaterialUIController,setMiniSidenav,
  setTransparentSidenav,setWhiteSidenav,} from "../../context";
...
export default function Silver_Items() {
...
const [notify, setNotify] = useState({ isOpen: false, message: '', type: '' ,variant:''})
  ...
  const {
    ...
    datas,
    TblHead,
} = useTable(data, filterFn,userService.headCellssilveritem);
    ...
    setFilterFn({
        fn: items => {
            if (target.value === "")
                ...
                x.itemName.toLowerCase().includes(target.value) || x.itemType.toLowerCase().includes(target.value)
...
const navigate = useNavigate();
  const Fetchuser=()=>{
      createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primaryfetchAllsilveritems()
...
Fetchuser()
  ...
  const Createuser=async()=>{
      createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primarycreateitem(values)
  ...
  const Updateuser=async()=>{
     createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primaryupdatesilveritem(values.id,values)
      ...
      var dataNew=data;
       ...
       dataNew.forEach(users=>{
        ...
        users.itemName=values.itemName;
          ...
          users.joins =   values.itemSize + "-" + "["+values.itemName+"]" + "-"+ values.itemType
...
const [deleting, setDeleting] = useState(false);
 const onDelete = async (id) => {
        setConfirmDialog(false)
       createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primarydeleteitem(id)
                   .then(res => {
                   FetchStk()
                   Fetchuser();
                resetForm()
    ...
    year: 'numeric',
             ...
             const newId = generateNewId();
              ...
              const updatedData = [...data.slice(0, itemIndex), ...data.slice(itemIndex + 1)];
               ...
               try {
                const response =  axios.post('https://serdb.onrender.com/api/DelSilverstk', deletedItem);
  ...
  const onDelete_backup = id => {
        setConfirmDialog(false)
       createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primarydeletesilveritem(id)
                   .then(res => {
                   Fetchuser();
                   FetchStk();
                resetForm()
...
// your function to copy here
  ...
  const validate = (fieldValues = values) => {
     ...
     const {
        ...
        handleChange,
        resetForm
    } = useForm(userService.initialsilveritems, true, validate);
...
const handleSubmit = e => {
        e.preventDefault()
        if (validate()) {
            createAPIEndpoint(ENDPIONTS.GSTSILVERSTOCK).primarycreatesilveritem(values)
  ...
  if (deleting) {
    intervalId = setInterval(async () => {
      ...
      try {
        ...
        await axios.delete(`https://serdb.onrender.com/api/GstSilverStock/${idToDelete}`);
...
const deletedItem = {
        ...
        await axios.post('https://serdb.onrender.com/api/DelSilverstk', deletedItem);
        console.log('Posted modified deleted item to DelSilverstk:', deletedItem);
...
const handleExport = () => {
  if (data.length > 0) {
    ...
    // Save the Blob as a file
    const fileName = 'SilverInventory.xlsx';
    saveAs(blob, fileName);
  } else {
    // If no data, export only the header
    const header = ['hireDate', 'HUID', 'itemName', 'itemWeight', 'itemPrice', 'itemSize', 'itemType'];
    ...
    return (
     item.HUID.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      item.itemName.toLowerCase().includes(searchTerm.toLowerCase()) ||
       ...
       item.itemPrice.toLowerCase().includes(searchTerm.toLowerCase()) ||
         item.hireDate.toLowerCase().includes(searchTerm.toLowerCase()) 
     
    );
  }).map((item) => ( {
        author:   new Date(item.hireDate).toLocaleDateString() === new Date().toLocaleDateString() ?
     <>
      <Checkbox {...label}  size="small" 
        ...
        <Chip size="small" label={  <Box fontWeight="fontWeightBold" fontSize={16}>{highlightSearchTerm(new Date(item.hireDate).toLocaleDateString())}</Box>}  color="error" variant="outlined"/>
      </>
  ...
  <ItemCompt description={highlightSearchTerm(item.hireDate)} />
        ...
        function: 
         <MDBox ml={-1}>
            <MDBadge  badgeContent={
                  <Job title={highlightSearchTerm(item.orderNumber)/*item.HUID */} />
      ...
      status: <ItemCompt description={highlightSearchTerm((item.itemName)+"″")} />,
        employed: (
          <MDBox ml={-1}>
            <MDBadge  badgeContent={
                  <Job title={highlightSearchTerm((item.itemPrice)+"/GMS")} />
...
<ThemeProvider theme={blueTheme}>
 <Button className={`${styles.roots}`} style={{ backgroundColor: "#ABD1FF"}}  onClick={() => EditOrRemove(item)}>
  ...
  <MDTypography variant="h6" color="white" onClick={handleExport}>
                  SILVER INVENTORY MANAGEMENT
                </MDTypography>

                  <MDTypography variant="h6" color="black">
                  NET WEIGHT ➼➼ {roundTo2DecimalPoint(silverstk).toFixed(3)}G 
                </MDTypography>
                       </MDBox>
                  </MDBox>
  ...
  <MDBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
        ...
        variant="gradient"
          color={sidenavColor}
          fullWidth
            startIcon={<AddTaskOutlinedIcon     onClick={openListOfOrders}/>} >
        Add
        </MDButton>
</MDBox>
  </MDBox>
      ...
      <Avatar  src={values.imageSrc} sx={{ width: 50, height: 50 ,bgcolor: 'white'}}>
        ...
        <IconButton color="info" aria-label="upload picture" component="span">
          <PhotoCamera />
            ...
            <Grid item xs={6}>
            <Autocomplete
     options={userService.itemTypesilver}
     freeSolo
     fullWidth
     onChange={handleChange}
...
/>
  ...
  <Grid item >
    <MDBox pt={2}>
        ...
        //color={sidenavColor}
        fullWidth
       startIcon={<RotateLeftOutlinedIcon onClick={resetForm}/>} >
        Reset
        </MDButton>
</MDBox>
</MDBox>
...
/>
  ...
  <TextField
        ...
        variant="gradient"
          color={sidenavColor}
          fullWidth
          startIcon={<UpdateDisabledOutlinedIcon onClick={() => Updateuser()}/>}
