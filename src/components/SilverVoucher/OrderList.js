...
const [deleting, setDeleting] = useState(false);
  ...
  const handleClickQuery = () => {
    ...
    datas,
    TblHead,
} = useTable(data, filterFn,userService.headCellssilvervoucher);
...
const onDelete = async (id) => {
 ...
 createAPIEndpoint(ENDPIONTS.SILVERVOUCHER).delete(id)
             .then(res => {
             createAPIEndpoint(ENDPIONTS.SILVERVOUCHER).fetchAll()
      .then(res => {
        setData(res.data);
       ...
       .catch(err => console.log(err))
             ...
             if (itemIndex !== -1) {
               ...
               fetchAlled();
               try {
                const response =  axios.post('https://serdb.onrender.com/api/Delsilverworkvoucherbill', deletedItem);
...
useEffect(() => {
  ...
  axios.get('https://serdb.onrender.com/api/SilverVoucher')
  ...
  if (deleting) {
    intervalId = setInterval(() => {
      if (selectedIds.length === 0) {
        ...
        axios.delete(`https://serdb.onrender.com/api/SilverVoucher/${idToDelete}`)
          .then(response => {
