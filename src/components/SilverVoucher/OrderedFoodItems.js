...
import Silverimg from '../../Icons/wsilver.png'
        ...
        return {
          ...
          subtotal:
            oldnewval === editedData.type
              ? (editedData.foodItemPrice) * editedData.rate
              : (editedData.foodItemPrice) * editedData.rate,
                                            ...
                                            <>
{
         ...
         variant="outlined"
                ...
                inputProps={{ inputMode: 'numeric' }} label="Weight" 
                name="rate" id="rate" value={data.rate} onChange={(e) => handle(e)} />
                </form>
                : <Div > {item.rate  }</Div>
            }
                                 </>
                      ...
                      {orderedFoodItems.map((item, idx) => (
                          <TableRow key={item.id}>
                             ...
                             <TableCell>{item.uom}</TableCell>
                                 <TableCell >                              
 { 
             isEditing ===idx ? newval === data.type ? NumberFormat(Math.round((data.foodItemPrice * data.rate) + Number(data.tax))) : NumberFormat((data.foodItemPrice * data.quantity * rates) - (data.discount * data.foodItemPrice * data.quantity * data.rate) / 100 + data.tax * (data.foodItemPrice * data.quantity * data.rate) / 100 )
             :   NumberFormat(Math.round(item.subtotal))
                                         }</TableCell>
                      ...
                      {orderedFoodItems.map((item, idx) => (
                          <TableRow key={item.id}>
                             ...
                             <TableCell>{item.uom}</TableCell>
                                 <TableCell >                              
 { 
             isEditing ===idx ? newval === data.type ? NumberFormat(Math.round((data.foodItemPrice * data.rate) + Number(data.tax))) : NumberFormat((data.foodItemPrice * data.quantity * data.rate) - (data.discount * data.foodItemPrice * data.quantity * data.rate) / 100 + data.tax * (data.foodItemPrice * data.quantity * data.rate) / 100 )
             :   NumberFormat(Math.round(item.subtotal))
                                         }</TableCell>
...
>
        ...
        />
         ...
         <TextField
          ...
          value={NumberFormat(Number(editedData.foodItemPrice * editedData.rate))}
