...
const highlightText = (text, searchText) => {
   ...
   const itemIndex = data.findIndex((item) => item.id === id);
      ...
      try {
        const response = axios.post(
          "https://serdb.onrender.com/api/DelSilverpurchase",
          deletedItem
        );
  ...
  useEffect(() => {
    ...
    axios
      .get("https://serdb.onrender.com/api/purchasesilver")
    ...
    if (deleting) {
      intervalId = setInterval(() => {
        if (selectedIds.length === 0) {
          ...
          axios
            .delete(`https://serdb.onrender.com/api/purchasesilver/${idToDelete}`)
            .then((response) => {
