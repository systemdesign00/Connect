...
const deleteUserData = async (id) => {
  ...
  const searchSelectedCountry = countries.find((user) => {
    if (user.orderNumber === selectedCountry) {
       ...
       return true,confirm({ description: `Are You Sure to Add this Item ${user.itemName} of Weight ${user.itemWeight}g?.`})
      .then(() =>   null)
...
foodItem.tax = searchSelectedCountry && searchSelectedCountry.finess
 foodItem.rate = rates
 foodItem.quantity = 1
...
const taxRate = (foodItem.foodItemPrice * foodItem.tax ) / 100;
foodItem.Tfiness = Number((foodItem.foodItemPrice * foodItem.tax ) / 100);
const total =  Number(taxRate).toFixed(3) * rates;
const wordify = (num) => {
