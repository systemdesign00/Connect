...
import { ClearAll, DeleteOutline, ResetTvOutlined } from '@mui/icons-material';
...
export default function Add_Stock(props, initialFValues) {
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && stockaddorderedFoodItems.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, stockaddorderedFoodItems])
    let today = new Date();
...
const tday = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        ...
        setValues({
            ...values,
            stockaddorderDetails: [...values.stockaddorderDetails, x]
        })
...
//foodItem.quantity = 1
foodItem.foodItemId = searchSelectedCountry && searchSelectedCountry.orderNumber
foodItem.foodItemName = searchSelectedCountry && searchSelectedCountry.itemName
foodItem.foodItemPrice =searchSelectedCountry && searchSelectedCountry.itemWeight
foodItem.tax = searchSelectedCountry && searchSelectedCountry.AddPrice
foodItem.huid = searchSelectedCountry && searchSelectedCountry.HUID
foodItem.rate = rates
foodItem.quantity = 1
...
const b = Number(foodItem.rate) + Number(foodItem.tax);
...
<TableContainer component={Paper}>
      <Table style={{ Width: 200 }} aria-label="customized table">
        <TableHead>
          <TableRow>
             <TableCell>Select Non_stock Item </TableCell>
    ...
    <TableCell>
          ...
          InputProps={{
          readOnly: true
        }} focused size="small" style = {{width: 100}}  label="Item Name" name="foodItemName" id="foodItemName" value={foodItem.foodItemName} onChange={handleInputChange} />
       </form>
       </TableCell>
        <TableCell>
          <form onSubmit={submitOrder}>
          ...
          readOnly: true
        }}  focused size="small" style = {{width: 100}}  label="HUID" name="huid" id="huid" value={foodItem.huid} onChange={handleInputChange} />
       </form>
       </TableCell>
        <TableCell>
          ...
          <TextField InputProps={{
          readOnly: true
        }} focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Weight" name="foodItemPrice" id="foodItemPrice" value={foodItem.foodItemPrice} onChange={handleInputChange} />
</form>
       </TableCell>
        <TableCell>
              <form onSubmit={submitOrder}>
         <TextField InputProps={{
          readOnly: true
        }} focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="+Rate" name="tax" id="tax" value={foodItem.tax }  onChange={handleInputChange} />
 ...
 { /*
  ...
  <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="taxs" name="taxs" id="taxs" value={oldfoodItem.taxs}  onChange={handleInputChange} />
    ...
    {/*  <Button  variant="contained" size="small" onClick={ resetInputField} ><DeleteOutline/></Button>*/}
