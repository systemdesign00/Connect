...
const generateNewId = () => {
  ...
  return Math.floor(Math.random() * 1000);
       ...
       createAPIEndpoint(ENDPIONTS.SIMPLEGST).delete(id)
                   .then(res => {
                   fetchAlled();
                   resetFormControls();
                   })
             ...
             if (itemIndex !== -1) {
              const newId = generateNewId();
               ...
               try {
                const response =  axios.post('https://serdb.onrender.com/api/Delgoldgstbill', deletedItem);
  ...
  if (deleting) {
    intervalId = setInterval(async () => {
      ...
      try {
        ...
        await axios.delete(`https://serdb.onrender.com/api/SimpleGST/${idToDelete}`);
        ...
        await axios.post('https://serdb.onrender.com/api/Delgoldgstbill', deletedItem);
...
const handleDeleteSelected = async () => {
 ...
 let oldcal =  Data?.oldgolditems?.reduce((total, currentValue) => total = total + Number(currentValue.subtotals),0);
 ...
 {/*
  ...
  <FormControl  sx={{ m: 3 }} fullWidth>
          <InputLabel htmlFor="outlined-adornment-amount">Search Ventor</InputLabel>
          <OutlinedInput
  ...
  <FormControl  sx={{ m: 3 }}>
    ...
    {
      //deleting ? successMessage && <div>{successMessage}</div> : ''
    }
...

{
  ...
  <> 
    <TableRow className={classes.tablerow}>
                        ...
                        <TableCell sx={{wordBreak:"break-all",padding: '8px'}}>{NumberFormat(Data.oldgoldamount)}</TableCell>
                           </TableRow>
   <TableRow className={classes.tablerow}>
                      <TableCell></TableCell>
                      <TableCell></TableCell>
...
{NumberFormat(Math.round(Number(newitem) + (((newitem*1.5)/100) + ((newitem*1.5)/100)))
                      - Number(Data.oldgoldamount))}</TableCell>
                    </TableRow>
  {
Data.oldgoldamount > Number(newitem) + (((newitem*1.5)/100) + ((newitem*1.5)/100)) ? 
    ""
: <TableRow className={classes.tablerow}>
                    <TableCell></TableCell>
                    <TableCell></TableCell>
                    ...
                    <TableCell sx={{wordBreak:"break-all",padding: '8px'}}>{NumberFormat((Number(Math.round(Number(newitem) + (((newitem * 1.5) / 100) + ((newitem * 1.5) / 100)))) 
                      - Number(Data.oldgoldamount)) - (Number(Data.gCash) + Number(Data.onlinecash)))}</TableCell>
</TableRow>
}
                  <TableRow className={classes.tablerow}>
                      <TableCell></TableCell>
