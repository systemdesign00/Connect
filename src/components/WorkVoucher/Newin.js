...
// eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        if (orderId === 0) resetFormControls()
            ...
            createAPIEndpoint(ENDPIONTS.WORKVOUCHER).fetchById(orderId)
                .then(res => {
                    setValues(res.data);
                    setErrors({});
                })
...
values.status = (Number(oldcal).toFixed(3))
...
values.balance = (Number(oldcal).toFixed(3));
///values.balance = (Number(values.scGst) - Number(oldcal)) - (Number(values.cashreceived) + Number(values.cashreceivedonline))
    const validateForm = () => {
        ...
        temp.secondarymobile = values.secondarymobile.length > 9 ? "" : "Minimum 10 numbers required!"
...
const submitOrderreset = e => {
    ...
    const submitOrder = e => {
        ...
        if (validateForm()) {
            if (values.id == 0) {
                createAPIEndpoint(ENDPIONTS.WORKVOUCHER).create(values)
                    .then(res => {
                      audio.play()
                       resetValues()
                      handleResetClick()
                ...
                createAPIEndpoint(ENDPIONTS.WORKVOUCHER).update(values.id, values)
                    .then(res => {
                      audioupdate.play()
                        setOrderId(0);
                         getFreshModelObject()
