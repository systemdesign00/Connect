...
export default function Rategcut(props, initialFValues) {
    ...
    useEffect(() => {
        ...
        x = x.filter(y => {
            return y.foodName.toLowerCase().includes(searchKey.toLocaleLowerCase())
                && orderedratecutitems.every(item => item.foodItemId != y.foodItemId)
        });
        setSearchList(x);
    }, [searchKey, orderedratecutitems])
   let today = new Date();
...
const tday = today.getDate() + "-"+ parseInt(today.getMonth()+1) +"-"+today.getFullYear();
    ...
    const addFoodItem = foodItem => {
        ...
        setValues({
            ...values,
            ratecutitems: [...values.ratecutitems, x]
        })
...
const fetchDatas = async () => {
      try {
        ...
        //  fetch data
        const dataUrls = `https://serdb.onrender.com/api/wholestock`;
...
const a = foodItem.foodItemPrice * 1;
//const taxRate = foodItem.tax ;
foodItem.tax = Number(foodItem.foodItemPrice / foodItem.rate).toFixed(3);
      ...
      <Table style={{ Width: 200 }} aria-label="customized table">
       ...
       <TableCell >Amount</TableCell>
        ...
        <TableCell>
          ...
          <TextField focused size="small" style = {{width: 100}}  label="Description" name="foodItemName" id="foodItemName" value={foodItem.foodItemName} onChange={handleInputChange} />
</form>
        ...
        <TableCell>
            <form onSubmit={submitOrder}>
          <TextField focused size="small" style = {{width: 100}} inputProps={{ inputMode: 'numeric' }} label="Amount" name="foodItemPrice" id="foodItemPrice" value={foodItem.foodItemPrice} onChange={handleInputChange} />
</form>
